...
cmds=OrderedDict([("avgmod","Calculate average modification level(AvgMod) of tested nucleotide(e.g. 5mC/6mA)"),\
                  ("shiftD","Shift the position of D in CIGAR in bam/sam. For deletion-induced techniques(e.g. BID-seq), if a deletion is detected in a polymer, it is re-assigned to the rightmost base of the polymer."),\
                  ...
                  ("mergeTSV","Transfer the transcriptome AvgMod file to genome positions, and then merge it with the genome AvgMod file. This function is designed for RNA modification sequencing."),\
...
def main(cmd=''):
    ...
    if cmd=="shiftD":
        ...
        if args.sam_path!=None:
            ...
            args.sam_path=""
        shiftD(Alignments=args.Alignments,output=args.out,convert_from_base=convert_from_base,sam_path=args.sam_path)
    ...
    if cmd=='mergeTSV':
        disp("mergeTSV Started")
        gtf = read_gtf(args.gtf)
        disp("gtf loaded")
        merge_tsv(args.transcriptomeAlignment,args.output,gtf,args.unlift,args.rvstrand)
        ...
        if tmp!=0:
            disp('mergeTSV Failed.')
        else:
            disp('mergeTSV Finished. Merged AvgMod in {}'.format(args.output+"_AvgMod_merged.tsv"))

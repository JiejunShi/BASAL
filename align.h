...
inline void SingleAlign::CountMismatch_new(bit64_t *q, bit32_t offset, bit64_t *s) {
    ...
    else {//4-nucleotide mapping, new rule
        //M2 = param.XC64(*s) | ((q[FIXELEMENT*2])>>offset);
        //M3 = param.M2_judge(M2);
        //M4 = (((~M3) & M2) | (M3 & ((*q)>>offset))) ^ (*s);
        M4 = (((~param.M2_judge(param.XC64(*s) | ((q[FIXELEMENT*2])>>offset))) & (param.XC64(*s) | ((q[FIXELEMENT*2])>>offset))) | ((param.M2_judge(param.XC64(*s) | ((q[FIXELEMENT*2])>>offset))) & ((*q)>>offset))) ^ (*s);
        //param.disp_bin64(*q);param.disp_bin64(*s);param.disp_bin64(param.XC64(*s));param.disp_bin64(q[FIXELEMENT*2]);
        //param.disp_bin64(M2);param.disp_bin64(M3);param.disp_bin64(M4);param.disp_bin64(q[FIXELEMENT]);
        //cout<<param.XM64(M4&((q[FIXELEMENT])>>offset))<<endl;
    	if((tmp_snp=N_count+param.XM64(M4 & ((q[FIXELEMENT])>>offset)))>snp_thres) return;
    	for(bit32_t i=1;i<FIXELEMENT;i++) {
            //M2 = param.XC64(s[i]) | (((q[FIXELEMENT*2-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT*2]>>offset);
            //M3 = param.M2_judge(M2);
            //M4 = (((~M3) & M2) | (M3 & (((q[i-1]<<1)<<(63-offset))|q[i]>>offset))) ^ s[i];
            M4 = (((~param.M2_judge(param.XC64(s[i]) | (((q[FIXELEMENT*2-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT*2]>>offset))) & (param.XC64(s[i]) | (((q[FIXELEMENT*2-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT*2]>>offset))) | (param.M2_judge(param.XC64(s[i]) | (((q[FIXELEMENT*2-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT*2]>>offset)) & (((q[i-1]<<1)<<(63-offset))|q[i]>>offset))) ^ s[i];
            /*
            cout<<"q[i-1]\t";param.disp_bin64(q[i-1]);
            cout<<"q[i]\t";param.disp_bin64(q[i]);
            cout<<"q[FE*2+i-1]\t";param.disp_bin64(q[FIXELEMENT*2-1+i]);
            cout<<"q[FE*2+i]\t";param.disp_bin64(q[FIXELEMENT*2+i]);
            cout<<"XC64(s[i])\t";param.disp_bin64(param.XC64(s[i]));
            cout<<"M2 right\t";param.disp_bin64(((q[FIXELEMENT*2-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT*2]>>offset);
            cout<<"M2\t";param.disp_bin64(M2);
            cout<<"M3\t";param.disp_bin64(M3);
            cout<<"~M3\t";param.disp_bin64(~M3);
            cout<<"M4 mid\t";param.disp_bin64(((q[i-1]<<1)<<(63-offset))|q[i]>>offset);
            cout<<"M4 left\t";param.disp_bin64(((~M3) & M2) | (M3 & (((q[i-1]<<1)<<(63-offset))|q[i]>>offset)));
            cout<<"s[i]\t";param.disp_bin64(s[i]);
            cout<<"M4\t";param.disp_bin64(M4);
            cout<<param.XM64(M4&(((q[FIXELEMENT-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT]>>offset))<<endl;
            */
    		if((tmp_snp+=param.XM64(M4&(((q[FIXELEMENT-1+i]<<1)<<(63-offset))|q[i+FIXELEMENT]>>offset)))>snp_thres) return;
    	}
    }
}
...
inline bit32_t SingleAlign::MismatchPattern0_new(bit64_t *q, bit64_t *s, bit32_t offset) {
    ...
    for(i=0;i<end_element;i++) {
		tmp=(s[i]<<offset)|((s[i+1]>>(63-offset))>>1);
        if(param.nt3){
            tmp=(q[i]^param.XT64(tmp));
        }else{
            //M2=param.XC64(tmp)|q[i+FIXELEMENT*2];
            //M3=param.M2_judge(M2);
            //tmp^=(((~M3) & M2) | (M3 & q[i]));
            tmp^=(((~param.M2_judge(param.XC64(tmp)|q[i+FIXELEMENT*2])) & (param.XC64(tmp)|q[i+FIXELEMENT*2])) | (param.M2_judge(param.XC64(tmp)|q[i+FIXELEMENT*2]) & q[i]));
        }
        j=(int)(i*SEGLEN)-1;
        while(tmp) {
            jj=__builtin_clzll(tmp)>>1; j+=jj+1;
...
inline bit32_t SingleAlign::MismatchPattern1_new(bit64_t *q, bit64_t *s, bit32_t gap_index, bit32_t offset) {
    register bit64_t tmp; register bit32_t shift;
    bit32_t *mm_array; int i,ii, j,jj, ss=0;
    //cout<<"in MMpattern()  gap_index:"<<gap_index<<endl;
    //cout<<"shift="<<(1-((int)gap_index%2)*2)*((int)gap_index+1)/2<<" offset="<<seglen_offset<<" end_element="<<end_element;
    ...
    //for(i=0;i<5;i++) disp_bfa64(param.swap_endian64(q[i])); cout<<endl;
    //for(i=0;i<5;i++) disp_bfa64(param.swap_endian64(s[i])); cout<<endl;
    //param.disp_bin64(q[i]);
    //param.disp_bin64(s[i]);
    mm_array=mm_index[gap_index];
    for(i=end_element,ii=0;i>=0;i--,ii+=SEGLEN) {
		tmp=(s[i]<<offset)|((s[i+1]>>(63-offset))>>1);
        if(param.nt3){
